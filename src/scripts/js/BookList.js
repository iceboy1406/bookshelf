...
class BookList {
    ...
    static async render(books, zeroBookMessage) {
        const filteredBooks = Shelf.selectedShelf == Shelf.type.finishedRead
            ? await BookStore.getFinishedRead(books)
            : await BookStore.getUnfinishedRead(books);
        const allBooksFromActiveShelf = Shelf.selectedShelf == Shelf.type.finishedRead
        ...
        if (bookList !== null) {
            ...
            if (filteredBooks.length === 0) {
                ...
                bookList.innerHTML = `<p class="message">${allBooksFromActiveShelf.length > 0
                    ? zeroBookMessage
                    : 'Tidak ada buku'} di rak ${Shelf.selectedShelf === Shelf.type.finishedRead
                    ? 'Selesai Dibaca'
